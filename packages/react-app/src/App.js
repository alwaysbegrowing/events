...
const columns = [{
  ...
  key: 'ags',
  render: (temp) => {
    const data = Object.keys(temp.args).map(key => ({ [key]: temp.args[key] }))
    const rows = data.map((name, index) => (<li key={index}>{JSON.stringify(name)}</li>))
    return <>
      {rows}
...
function App() {
  ...
  return (
    <Table rowKey="logIndex" columns={columns} dataSource={events} />
